...
module timeRotorOffsetLegsAroundCenterPoint(legCount = 1, legLength = false, includeTriangleSupports = true, legAngle) {
    ...
    for (i = [0:legCount - 1]) {
        jointLeg = [legInstance(rotation = [0, 180 - legAngle, 0], insideDiameter = horizontalPipeInsideDiameter,
        pinHole = true, length = legLength)];
        rotate([0, 0, legSpacingInDegrees * i])
            joint(legs = jointLeg, sphereJointInsideDiameter = 0, closeUpWithSphere = false, addCircleToFlatBottom =
            false, jointPinHoleOffset = 40);
    }
}
...
module TimeRotorHolder(legCount = 6, insideDiameter = 45, outsideRingDiameter = 50, ringHeight = 100, ringPosition = -
50, legAngle = 10, legLength = 80, includeLegs = false) {
    difference()
        {
            union()
                {
                    timeRotorHolderCenter(supportHeight = horizontalPipeInsideDiameter + (jointWallThickness * 2),
                    outsideRingDiameter = outsideRingDiameter, insideDiameter = insideDiameter, ringHeight = ringHeight,
